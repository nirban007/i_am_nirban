...
class FaceRecognition:
    ...
    # """
    # Function to recognize faces and put text with a rectangle

    # Arguments: 
    #     embeddings: dict: Person's name with their embeddings
    #     cam_id: int: camera id of your device
    ...
    # """
        ...
        while True:
            ...
            # frame = frame[:, :, ::-1]
            # imgS = cv2.resize(frame, (frame.shape[0] // 4, frame.shape[1] // 4))
            # imgS = cv2.cvtColor(imgS, cv2.COLOR_BGR2RGB)
            imgS = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

            # take note of the start of the process, this will be used to track the entry time
            process_start = datetime.now() # process time initiate
            # print(f"Start of the process: {process_start}")
                ...
                for face in faces:
                    ...
                    for name, known_embedding in embeddings.items():
                        ...
                        if max_similarity >= thresh:
                            ...
                            # print(f"End of the process: {process_end}")
                            
                            # now send the two process times to calculate the elapsed time to recognize the person
                            # then subtract the elapsed time from the end process time to get to the 
                            # initial entry time and mark that time
                            
                            process_end = datetime.now() # process time end
                            shifted_entry = self._calculte_shifted_entry(process_start, process_end) # calculate shifted entry
                            
                            self._mark_attendance(name, shifted_entry) # mark the attendance based on process times
                            self._save_image(save_dir, frame, shifted_entry) # save the images at shifted entry time
                        else:
                            continue
    ...
    def _save_image(self, path, image, marked_time):
        ...
        dir_path = "D:\\Projects\\who-are-you\\face_recognition\\Employee\\captured"
        ...
        # Save the file at dir_path with marked_time as part of the file name
        frame_name = os.path.join(dir_path, f"frame_{marked_time_str}.jpg") 

        try:
            cv2.imwrite(frame_name, image)  # Saving the file
